...
variable "identities" {
  description = <<-EOT
    ...
    a governed change and binds a dedicated invoke-only trigger service
...
variable "break_glass_recovery" {
  description = <<-EOT
    The approved recovery binding of the approved zone: the per-activation

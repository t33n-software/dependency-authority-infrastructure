...
Authority Infrastructure source boundary. They do not define a second release,
evidence, or dependency policy.

## Import timing

Import through the GitHub graphical interface only after the first DAI-1 pull
...
verification, recursive format check, and init plus validate for every module,
the policy bindings and every reference stack with enforced provider GPG
validation.
